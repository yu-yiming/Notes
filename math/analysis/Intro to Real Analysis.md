...
现在定义：
$$
f^+(x) = \inf_{\delta > 0}\sup\{f(y)\mid |x - y| < \delta\}
$$
有下列引理：

> **引理**：
> $$
> f^+(x) = \sup\{\lim_{x_k \to x} f(x_k) \mid f(x_k)\ \text{收敛}\} = \sup\{\lim\sup f(x_k)\}
> $$
> 

### 连续性

首先让我们回忆函数相关的概念：函数 $f$ 有定义的集合被称为它的 **定义域（Domain）**，记作 $\dom{f}$。实分析研究的函数均满足 $\dom{f} \subseteq \mathbb{R}$，也即 **实值函数（Real-Valued Function）**。通常我们会将函数实数域中最大的有定义子集作为其定义域，比如 $f(x) = \frac{1}{x}$ 的定义域通常是 $\{x \in \mathbb{R} \mid x \ne 0\}$，$g(x) = \sqrt{4 - x^2}$ 的定义域通常是 $[-2, 2]$，所以如果没有特别说明，都是取默认的定义域。

函数的一个重要性质是 **连续性（Continuity）**。一个非常直观的定义是，如果对于函数上某点 $x$，所有收敛于 $x$ 的实数列 $\{x_n\}, x_n \in \dom{f}$ 都满足 $\lim f(x_n) = f(x)$，则称函数在 $x$ 处是 **连续的（Continuous）**；如果函数 $f$ 在其定义域中处处连续，我们就将它称为 **连续函数（Continous Function）**。一个更加严格的定义需要用到 $\epsilon-\delta$：

> **定理**：设 $f$ 是一个定义域为 $\mathbb{R}$ 子集的实值函数，则它在点 $x$ 处连续当且仅当对于任意的 $\epsilon > 0$ 都存在 $\delta > 0$ 使得任意 $y \in \dom{f}$ 且 $|y - x| < \delta$ 都能令 $|f(y) - f(x)| < \epsilon$ 成立。

> **证明**：$\lackproof$

下面我们将从另一个角度审视连续性这个概念。回忆我们此前定义的 $f^+$，其含义大致是函数某一点去心邻域中函数取值的下确界，也就是 $f$ 中 $x$ 以外的点能取到的 比 $f(x)$ 更大的最小值。我们定义 $f$ 在 $x$ 处 **上半连续（Upper Semi-continuous）** 如果 $f^+(x) \le f(x)$。其等价的另一个定义是：对于任意收敛于 $x$ 的数列 $\{x_n\}$ 都有：
$$
\underset{x_n \to x}{\lim \sup} f(x_n) \le f(x)
$$
这两个定义是等价的，接下来证明这一点：
...
随后我们只需要对称地定义：
$$
f^-(x) = \sup_{\delta > 0}\inf\{f(y) \mid |x - y| < \delta\}
$$
以及相应的 **下半连续（Lower Semi-continuous）**。就可以得到函数连续性的定义：若函数 $f$ 在 $x$ 点同时上半连续且下半连续，则它在该点 **连续（Continuous）**。这和我们给出的前两个定义是等价的，下面是证明：

> **证明**：$\lackproof$

半连续的的区间可以暗示函数在该区间的极值性质：

> **定理**：如果 $f$ 在 $[a, b]$ 区间内上半连续，则它在该区间内有最大值。

> **证明**： $\lackproof$

### 度量空间

函数的极值问题常常是我们非常关心的内容，比如拉格朗日乘数法就用来计算特定约束下函数的最大值。前面已经提到，一元函数在上半连续的开区间中一定能找到最大值；现在我们需要将这个性质推广到多维空间中。多维空间中什么样的集合能够让我们得到类似的性质呢？我们称这样的集合为 **紧致集（Compact Set）**，随后会介绍到。而紧致集定义的空间被称为 **度量空间（Metric Space）**。下面是具体定义：

> **定义**：度量空间 $(X, d)$ 是定义了 $d : X \times X \to [0, \infty)$ 的集合，其中：
>
> - $d(x, y) = 0$ 当且仅当 $x = y$。
> - $d(x, y) \le d(x, z) + d(z, y)$。
> - $d(x, y) = d(y, x)$。

这里的函数 $d$ 可以理解为距离函数，在 $\mathbb{R}^n$ 中可以简单理解为空间中两点的直线距离。
